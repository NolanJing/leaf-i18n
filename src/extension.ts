...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.commands.registerCommand('vscode-i18n-linter.extractI18N', args => {
      return new Promise(resolve => {
        ...
        if (args.isCommon) {
          inputBoxParams = {
            value: `[${moduleName}]->`,
            promptTxt: '`[common]->key 或者 [common]->name.key`',
            reg: /(\[common\]->\w+)|(\w+\.\w+)/
          };
        }
        ...
        return resolve(
          vscode.window.showInputBox({
            prompt: `请输入变量名，格式${
              inputBoxParams.promptTxt
            }，按 <回车> 启动替换 `,
            value: inputBoxParams.value,
            validateInput(input) {
              if (!input.match(inputBoxParams.reg)) {
                return `变量名，格式  ${inputBoxParams.promptTxt}`;
              }
            }
          })
        );
        ...
        const finalArgs = Array.isArray(args.targets)
          ...
          : [args.targets];
        if (
          finalLangObj[val] !== undefined &&
          finalLangObj[val] !== finalArgs[0].text
        ) {
          vscode.window.showInformationMessage(
            `${val}已经存在，值为${finalLangObj[val]}，无法覆盖`
          );
          return;
        }
        return finalArgs
          .reduce((prev: Promise<any>, curr: TargetStr, index: number) => {
            return prev.then(() => {
              ...
              return replaceAndUpdateInVue(curr, val, isExsit);
            });
          }, Promise.resolve())
          .then(
            () => {
              vscode.window.showInformationMessage(
                `成功替换本文件内的 ${finalArgs.length} 处文案`
              );
            },
            err => {
              console.log(err, 'err');
  ...
  function watchCurrentI18NFile() {
    dirI18nWatcher = vscode.workspace.createFileSystemWatcher(
      getCurrentDirI18NPath()
    );

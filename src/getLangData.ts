...
import { I18N_GLOB_PATH, exDirNameList } from './const';
/**
 * 获取文件 Json
 */
function getLangJson(fileName) {
  ...
  if ((jsObj as any)['zh-CN']) {
    return (jsObj as any)['zh-CN'];
  } else {
    return {};
  }
}
...
export function getSuggestLangObj() {
  ...
  currentFileSplits.push('i18n.ts');
  const dirI18nPath = currentFileSplits.join('\\'); // 拿到当前文件夹下的i18n.ts路径
  const langObj = {

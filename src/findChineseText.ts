...
function findTextInVue(code, fileName) {
  ...
  let cpCode = code
  // let scriptCode = cpCode.split(/\<\/*script\>/)[0]
  // let scriptOffset = code.indexOf('<script lang="ts">') + '<script lang="ts">'.length // script里面代码的偏移量

  let scriptCode = cpCode.match(/<script\b[^>]*>[\s\S]*<\/script>/)[0]
  let scriptOffset = code.indexOf('<script lang="ts">')   // script里面代码的偏移量
  const ast = ts.createSourceFile(
    '',
    scriptCode,
    ts.ScriptTarget.ES2015,

  ...
  if (keyName.startsWith(`[${COMMON}]`)) {
    ...
    if (!fs.existsSync(globalDirPath) || !fs.existsSync(I18N_GLOB_PATH)) {
      ...
      vscode.window.showInformationMessage(
        `成功新建语言文件 ${I18N_GLOB_PATH}`
      );
    }
    let fixKeyName = keyName.replace(`[${COMMON}]->`, '');
    updateGlobalLanguageFile(fixKeyName, text);
  } else {
    let currentDirI18nPath = getCurrentPath() + SEP + 'i18n.ts';
    if (!fs.existsSync(currentDirI18nPath)) {
      fs.writeFileSync(currentDirI18nPath, getDirLanguageFileContent());
      vscode.window.showInformationMessage(
        `成功新建语言文件 ${currentDirI18nPath}`
      );
    }
...
export function updateGlobalLanguageFile(keyName: string, text: string) {
  ...
  if (moduleKeys.length == 2) {
    ...
    realKey = moduleKeys[1];
    if (!currentLangObj['zh-CN'][moduleName]) {
      currentLangObj['zh-CN'][moduleName] = {};
    }
    currentLangObj['zh-CN'][moduleName][realKey] = text;
  }
  // common.sub.key
  // if(moduleKeys.length == 3){
  //   moduleName = moduleKeys[0]
  //   subModuleName = moduleKeys[1]
  //   realKey = moduleKeys[2]
  //   if (!currentLangObj['zh-CN'][moduleName]) {
  //     currentLangObj['zh-CN'][moduleName] = {}
  //   }
  //   if (!currentLangObj['zh-CN'][moduleName][subModuleName]) {
  //     currentLangObj['zh-CN'][moduleName][subModuleName] = {}
  //   }
  //   currentLangObj['zh-CN'][moduleName][subModuleName][realKey] = text
  // }

  fs.writeFileSync(
    I18N_GLOB_PATH,
    getNewGlobalLanguageFileContent(currentLangObj)
  );
}
...
export function updateDirLanguageFile(keyName: string, text: string) {
  ...
  fs.writeFileSync(
    currentDirI18nPath,
    getDirLanguageFileContent(currentLangObj)
  );
}
 ...
 * 对 dialogs form 等 特殊处理
 */
export function getCurrentPath(hasExDirName = true) {
  let activeEditor = vscode.window.activeTextEditor;
  let currentFileSplits = activeEditor.document.fileName.split(SEP);
  currentFileSplits.pop();
  if (hasExDirName) {
    exDirNameList.forEach((exDirName: string) => {
      if (currentFileSplits[currentFileSplits.length - 1] == exDirName) {
        currentFileSplits.pop();
      }
    });
  }
